...
describe('Product', () => {
  ...
  it('should decrement amount', () => {
    const wrapper = mount(<Product product={product} />);
    wrapper.find('button[data-id="product-increment"]').simulate('click').simulate('click').simulate('click');
    wrapper.find('button[data-id="product-decrement"]').simulate('click').simulate('click');
    expect(wrapper.find('[data-id="product-amount"]').text()).toBe('1');
  });
